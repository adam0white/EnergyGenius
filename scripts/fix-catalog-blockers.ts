...
function fixCatalog(): { fixed: SupplierPlan[]; stats: FixStats } {
  ...
  const fixed = supplierCatalog.map((plan, index) => {
...
function validateFixes(plans: SupplierPlan[]): boolean {
  ...
  if (!duplicateCheck) {
    ...
    const remaining = Array.from(idCounts.entries()).filter(([_, count]) => count > 1);
  ...
  console.log(`✓ Valid contract terms: ${termsCheck ? 'All plans >= 1 month' : `${invalidTerms.length} plans with 0 months`} ${termsCheck ? '✓ PASS' : '✗ FAIL'}`);
