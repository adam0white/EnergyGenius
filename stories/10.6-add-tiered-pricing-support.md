...
interface SupplierPlan {
  ...
  supplier: string;
  planName: string;
  baseRate: number;        // ← Only ONE rate per plan
  monthlyFee: number;
  contractTermMonths: number;
  // ❌ NO support for tier structure
}
```
...
interface SupplierPlan {
  ...
  supplier: string;
  planName: string;
  baseRate: number;         // ← Keep for flat-rate plans
  monthlyFee: number;
  contractTermMonths: number;
  ...
  tiers?: PriceTier[];       // ← Optional: for tiered plans
  isTiered?: boolean;        // ← Flag: true if plan uses tiers
}
```
 ...
 */
function calculateTieredAnnualCost(
  plan: SupplierPlan,
  annualKwh: number
): number {
...
assert(cost === 1319.40); // (0.12 * 11000) + (9.95 * 12)

// Test 2: Tiered plan, user in tier 2
plan = {
  monthlyFee: 9.95,
  tiers: [
    { minKwh: 0, maxKwh: 500, rate: 0.10 },
...
const estimatedAnnualCost =
  (plan.baseRate * totalAnnualUsage) + (monthlyFee * 12);
```
...
{plan.isTiered && plan.tiers ? (
  <div className="text-xs text-gray-500 space-y-1">
    ...
    {plan.tiers.map((tier, idx) => (
      <div key={idx}>
        ${tier.rate}/kWh (${tier.minKwh}-{tier.maxKwh ?? '∞'} kWh)
      </div>
    ))}
  </div>
) : (
  <div className="text-sm text-gray-600">
    Flat Rate: ${plan.baseRate}/kWh
  </div>
)}
```
